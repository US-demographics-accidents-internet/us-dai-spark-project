...
def load_dataframes(spark: SparkSession, base_path: str):
    ...
    demographics_schema = StructType([
        StructField(f, IntegerType() if f in int_fields else FloatType() if f in float_fields else StringType(), True)
    ...
    print("PUSA:")
    # First 10 columns
    df_pusa.select(df_pusa.columns[:10]).show(5)
    print("PUSB:")
    df_pusb.select(df_pusb.columns[:10]).show(5)
    print("Accidents:")
    # First 10 columns
